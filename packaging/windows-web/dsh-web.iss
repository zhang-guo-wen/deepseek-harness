...
;
; 插件加载走 DeepSeek Harness 的默认机制：DSH_HOME 固定为用户的 ~/.dsh，
; web profile 自身的 cordis.patch.yml 挂载用户插件，与源码版共用一个家园。
; 托盘宿主（DeepSeek Harness.exe）是唯一入口；"纯净启动"由它在运行时用
; 一个临时空 DSH_HOME（<install>\clean-data）启动实现。
;
...
; No auto-run on install finish: the harness first launch heals its built-in
; packages into the home's module fallback, and launching it from the
; installer's postinstall step races that heal (the module fallback isn't ready
; yet), which the guardian then restarts in a loop. The user starts it by
; double-clicking the shortcut / DeepSeek Harness.exe once the install finishes.

[UninstallDelete]
; Only remove the install tree; the user DSH_HOME (~/.dsh) persists.
