...
describe('mcp-config', () => {
  it('converts a stdio spec without a plugin-owned description', () => {
    ...
    expect(config).toEqual({
      ...
      args: ['/c', 'npx', '-y', '@upstash/context7-mcp'], env: {}, cwd: '',
    })
  })
  ...
  it('maps http and sse specs onto the streamable-http config', () => {
    ...
    expect(mcpEntryConfig({ type: 'streamable-http', url: 'https://x/stream' }, 'stream1'))
      .toMatchObject({ url: 'https://x/stream' })
    expect(mcpEntryConfig({ type: 'stdio', command: 'cmd' }, 'stdio-defaults')).toMatchObject({ args: [], env: {}, cwd: '' })
  })

  it('rejects a stdio spec without a command', () => {
    expect(() => mcpEntryConfig({ type: 'stdio' } as never, 'x')).toThrow(/command/)
  })

  it('rejects a http spec without a url', () => {
    expect(() => mcpEntryConfig({ type: 'http' } as never, 'x')).toThrow(/url/)
  })
  ...
  it('round-trips a stored config back to the form spec', () => {
    const config = mcpEntryConfig({ type: 'stdio', command: 'cmd', args: ['a'] }, 'x')
